...
def _prediction_reasoning(
    *,
    ...
    lines = [
        f"- `risk_score` is the model-estimated fraud risk probability/score: `{risk_score:.4f}`.",
        f"- `predicted_label = {predicted_label}` means the model {'flagged' if predicted_label == 1 else 'did not flag'} potential fraud.",
        f"- `severity` maps the score into low / medium / high / critical; this result is classified as `{severity}`.",
        "- The conclusion is based on the model output plus the configured decision thresholds.",
    ]
    if threshold is not None:
        comparison = "at or above" if risk_score >= threshold else "below"
        lines.insert(1, f"- The configured decision threshold is `{threshold_text}`.")
        lines.append(
            f"- This score is {comparison} the threshold, so the API converted it into the predicted label above."
        )
    else:
        lines.insert(1, "- The configured decision threshold is read from model metadata.")
    return lines
...
decision_threshold = metadata.get("threshold") if "error" not in metadata else None
threshold_display = _format_threshold(decision_threshold if isinstance(decision_threshold, (int, float)) else None)
...
with st.sidebar:
    ...
    st.markdown("1. Start the full demo (`make demo`)")
    st.markdown("2. Or use Docker (`make demo-docker`)")
...
with st.expander("How this demo works", expanded=True):
    st.markdown(
        "\n".join(
            [
                "- This dashboard simulates a fraud risk monitoring workflow.",
                "- The data comes from the IEEE-CIS Fraud Detection dataset.",
                "- The model was trained offline and is served through FastAPI.",
                "- The dashboard sends transactions to the API and displays the returned risk results.",
                "- Alerts are generated by a rule-based engine that uses model outputs plus operational rules.",
                "- This is a local portfolio demo, not a production banking system.",
            ]
        )
    )
...
with st.container(border=True):
    ...
    k4.metric("Threshold", threshold_display)
...
with st.container(border=True):
    ...
    if "error" in metadata:
        ...
        c4.metric("Threshold", threshold_display)
...
with st.container(border=True):
    ...
    with st.expander("Where do the data fields come from?", expanded=False):
        st.markdown(
            "\n".join(
                [
                    "- `TransactionAmt` is the transaction amount.",
                    "- `ProductCD` is an anonymized product/category code from the IEEE-CIS dataset.",
                    "- `card1` through `card6` are anonymized card-related features.",
                    "- Many production fraud systems use anonymized and engineered fields like these.",
                    "- The demo uses a small subset of inputs so the UI stays understandable.",
                ]
            )
        )
    ...
    with st.form("single_prediction"):
        ...
        transaction_amt = col1.number_input(
            "TransactionAmt",
            min_value=0.0,
            value=120.5,
            help="Transaction amount in currency units. Larger values can attract more attention, but the model score drives the final output.",
        )
        product_cd = col2.selectbox(
            "ProductCD",
            options=["W", "H", "C", "S", "R"],
            index=0,
            help="Anonymized product/category code from the IEEE-CIS dataset.",
        )
        card1 = col3.number_input(
            "card1",
            min_value=0,
            value=1000,
            step=1,
            help="An anonymized card-related feature from the dataset, not a real card number.",
        )

        with st.expander("Optional card fields"):
            st.caption("These fields are anonymized IEEE-CIS features. Keep them collapsed unless you want to show the fuller payload.")
            o1, o2, o3 = st.columns(3)
            card2 = o1.number_input("card2", min_value=0.0, value=0.0)
            card3 = o2.number_input("card3", min_value=0.0, value=0.0)
            card4 = o3.text_input("card4", value="visa", help="An anonymized card network field from the dataset.")
            o4, o5, o6 = st.columns(3)
            card5 = o4.number_input("card5", min_value=0.0, value=0.0)
            card6 = o5.text_input("card6", value="credit", help="An anonymized card type field from the dataset.")
            include_optional = o6.checkbox("Include optional fields", value=False, help="Include the optional anonymized fields in the API payload.")
    ...
    if submitted:
        ...
        if "error" in prediction:
            ...
            with st.expander("Why this result?", expanded=True):
                st.markdown(
                    "\n".join(
                        _prediction_reasoning(
                            risk_score=risk_score,
                            predicted_label=raw_label,
                            severity=severity,
                            threshold=decision_threshold if isinstance(decision_threshold, (int, float)) else None,
                        )
                    )
                )
...
with st.expander("Batch Prediction (Optional)", expanded=False):
...
with st.container(border=True):
    st.subheader("Alerts")
    with st.expander("How to read alerts", expanded=False):
        st.markdown(
            "\n".join(
                [
                    "- Alerts are not raw model predictions only.",
                    "- The alert engine adds operational context: `reason_codes`, `recommended_action`, and `severity`.",
                    "- Reason codes are rule-based so the output is easier for an analyst to review.",
                ]
            )
        )
    ...
    if st.button("Generate Alert From Sample Transaction"):
        ...
        if "error" in alert:
            ...
            st.json(alert)
            st.info(
                "Alerts combine model output with rule-based severity, reason codes, and a recommended action so the result is easier to review."
            )
        ...
        st.markdown(
            "\n".join(
                [
                    ...
                    "- PR-AUC: especially useful for imbalanced fraud problems.",
                ]
            )
        )
...
with st.container(border=True):
    ...
    with st.expander("How to read threshold trade-offs", expanded=False):
        st.markdown(
            "\n".join(
                [
                    "- Lower threshold means more alerts and higher recall but more false positives.",
                    "- Higher threshold means fewer alerts and potentially higher precision but missed fraud risk.",
                    "- In fraud systems, threshold selection is a business decision, not only a model decision.",
                ]
            )
        )
...
with st.expander("Known limitations of this demo", expanded=False):
    st.markdown(
        "\n".join(
            [
                "- Local demo only.",
                ...
                "- JSONL alert storage is for local demo only.",
                "- The model is a baseline; more advanced models could be evaluated later.",
            ]
        )
    )
